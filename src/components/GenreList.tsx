            ...
            <List>
                {data?.results.map((genre) => <ListItem key={genre.id} paddingY="5px">
                                        <HStack>
                                            ...
                                            <Button whiteSpace="normal" textAlign="left"  fontWeight={ genre.id === selectedGenreId ? "bold" : ""}  variant="link" fontSize="lg" onClick={() => onSelectGenre(genre)}> {genre.name} </Button>
                                        </HStack>
                                      </ListItem>)}
            </List>
        </>
